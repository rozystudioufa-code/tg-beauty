...
Статические файлы (`admin/index.html`, `admin/app.js`, `admin/styles.css`) лежат в репозитории рядом с бэкендом. Fastify отдаёт их по роуту:
...
Доступ: `https://api.yourdomain.com/admin` — открывается в браузере на ноутбуке.

### Авторизация

Отдельный поток — не через Telegram initData (панель открывается вне Telegram):

1. При открытии `/admin` — страница входа: поле «Введите ваш Telegram ID»
2. Сервер сверяет с `PLATFORM_ADMIN_TELEGRAM_ID` из `.env`
3. Если совпадает → выдаёт admin-сессию (`httpOnly cookie`, TTL 8 часов)
4. Все запросы к `/admin/api/*` проверяют наличие валидной сессии → иначе `403`

> Второй фактор не нужен: `telegram_id` известен только тебе, и панель не публична.

---

### Экраны и структура

Панель — одностраничное приложение (vanilla JS, без фреймворков, в стиле текущего tg-app).

**Боковая навигация (всегда видна):**
```
● Дашборд
○ Мастера
○ Платежи
...
Четыре карточки статистики + два списка:

```
┌──────────────┐ ┌──────────────┐ ┌──────────────┐ ┌──────────────┐
│   Мастеров   │ │   Активных   │ │  Stars этот  │ │  Записей     │
│     12       │ │   Pro: 7     │ │  мес: 8 750  │ │  сегодня: 4  │
└──────────────┘ └──────────────┘ └──────────────┘ └──────────────┘

Истекают подписки (следующие 7 дней):      Новые мастера (последние 7 дней):
• @nail_master_ufa — 29.05 [Продлить]      • @brows_karina — зарег. 20.05
• @brow_studio     — 30.05 [Продлить]      • @lash_julia   — зарег. 18.05
```

API: `GET /admin/api/stats` — возвращает все цифры одним запросом.

---

#### Список мастеров (`/admin/masters`)

Таблица с фильтрами:

```
Поиск: [______________]   Тариф: [Все ▼]   Статус: [Все ▼]

Имя          Бот                  Тариф   Истекает    Услуг   Действие
─────────────────────────────────────────────────────────────────────
Разиля       @lashmaster_bot      PRO     15.06.26    8       [Открыть]
Алина        @nails_alina         PRO     29.05.26    6       [Открыть]
Карина       @brows_karina        Free    —           3       [Открыть]
Юлия         @lash_julia_ufa      PRO     01.07.26    5       [Открыть]
```

Строка со скоро-истекающей подпиской подсвечивается жёлтым (< 3 дней), красным (истекла).

API: `GET /admin/api/masters?search=&tier=&page=1`

---

#### Карточка мастера (`/admin/masters/:id`)

```
← Назад к списку

[Аватар]  Алина Иванова
          @nails_alina · telegram_id: 123456789
          Бот: @nailmaster_alina_bot
          Зарегистрирована: 01.04.2026

──────────────────────────────────────────
Подписка
  Тариф:     PRO  ⚠ истекает через 2 дня (29.05.2026)
  [Продлить Pro на 30 дней]   [Перевести на Free]

──────────────────────────────────────────
Активность
  Услуг активных: 6    Записей всего: 43    Записей в этом месяце: 8

──────────────────────────────────────────
История подписок
  Дата         Тариф   Срок       Stars   Источник
  01.05.2026   Pro     30 дней    250     Telegram Stars
  01.04.2026   Pro     30 дней    250     Telegram Stars
  01.03.2026   Pro     30 дней    250     Telegram Stars
```

**Кнопка «Продлить вручную»** — вызывает `POST /admin/api/masters/:id/extend`:
```json
{ "days": 30 }
```
Сервер обновляет `subscription_expires_at`, пишет запись в `subscriptions` с `payment_ref = 'manual_admin'`, отправляет мастеру сообщение в бот.

**Кнопка «Перевести на Free»** — вызывает `POST /admin/api/masters/:id/downgrade`. Сервер выполняет ту же логику что и `subscription-downgrade` cron, но немедленно.
...
```
Период: [Май 2026 ▼]          Итого за период: 8 750 Stars

Дата        Мастер              Stars   Тип              Статус
────────────────────────────────────────────────────────────────
20.05.2026  @lash_julia_ufa     250     Telegram Stars   ✓ Активирован
18.05.2026  @nails_alina        250     Telegram Stars   ✓ Активирован
10.05.2026  @lashmaster_bot     250     Ручное (admin)   ✓ Активирован
```

API: `GET /admin/api/payments?month=2026-05`

---

### Admin API — эндпоинты

Все защищены admin-сессионной cookie. Prefix: `/admin/api`.

| Метод | Путь | Что делает |
|-------|------|------------|
| `GET` | `/stats` | Дашборд: мастера, подписки, Stars, записи сегодня, скоро-истекающие |
| `GET` | `/masters` | Список мастеров с пагинацией и фильтрами |
| `GET` | `/masters/:id` | Детали мастера + история подписок |
| `POST` | `/masters/:id/extend` | Продлить подписку вручную на N дней |
| `POST` | `/masters/:id/downgrade` | Немедленно перевести на Free |
| `GET` | `/payments` | История платежей с фильтром по месяцу |

---

### Порядок разработки панели

Делать последней (шаг 14) — после того как весь API уже работает, данные реальные:

1. Статика: `admin/index.html` — боковое меню + роутинг через `history.pushState`
2. Авторизация: страница входа → cookie-сессия
3. Дашборд: `GET /admin/api/stats` → рендер карточек и списков
4. Список мастеров: таблица + поиск + фильтр + пагинация
5. Карточка мастера: детали + кнопки продления/даунгрейда
6. Платежи: таблица с фильтром по месяцу
...
13. **Admin панель** — статика в `/admin`, cookie-сессия, дашборд, список мастеров, карточка с ручным продлением/даунгрейдом, платежи
14. **Тестирование** — TESTING.md обновить под API-режим; отдельно тестировать race condition (два параллельных запроса на один слот)
